...
def simulate_samples(
    n_samples: int, is_for_training: bool, voltage_noise_stdv: float = 0.5
) -> pd.DataFrame:
    samples_per_scan = 8
    ...
    a = np.vectorize(lambda x: MINE_EFFECT.get(x)[0])(mine_type)
    b = np.vectorize(lambda x: MINE_EFFECT.get(x)[1])(mine_type)
    voltage = - a * heights + b
